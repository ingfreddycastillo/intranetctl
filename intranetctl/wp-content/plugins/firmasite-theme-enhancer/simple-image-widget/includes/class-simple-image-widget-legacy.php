 ...
 * @since 4.0.0
 */
class FirmaSite_Simple_Image_Widget_Legacy {
	/**
	 * Load legacy support.
	 *
	 * @since 4.0.0
		...
		if ( is_simple_image_widget_legacy() || ! empty( $instance['image'] ) ) :
			...
			<div class="simple-image-widget-legacy-fields">
				<?php if ( ! is_simple_image_widget_legacy() ) : ?>
					...
					<p>
						<em><?php _e( 'Select an image, then clear these values, and they will disappear when you save the widget.', "firmasite-theme-enhancer" ); ?></em>
					</p>
				<?php endif; ?>

				<p>
					<label for="<?php echo esc_attr( $widget->get_field_id( 'image' ) ); ?>"><?php _e( 'Image URL:', "firmasite-theme-enhancer" ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $widget->get_field_id( 'alt' ) ); ?>"><?php _e( 'Alternate Text:', "firmasite-theme-enhancer" ); ?></label>
	 ...
	 *
	 * Called in FirmaSite_Simple_Image_Widget::update().
	 *
	 * @since 4.0.0
	 *
