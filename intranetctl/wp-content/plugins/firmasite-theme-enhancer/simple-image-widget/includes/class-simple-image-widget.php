 ...
 * @since   3.0.0
 */
class FirmaSite_Simple_Image_Widget extends WP_Widget {
	/**
	...
	public function __construct( $id_base = false, $name = false, $widget_options = array(), $control_options = array() ) {
		...
		$name    = ( $name ) ? $name : __( 'Image', "firmasite-theme-enhancer" );

		$widget_options = wp_parse_args(
			$widget_options,
			array(
				'classname'   => 'widget_simpleimage', // Legacy class name.
				'description' => __( 'An image from your Media Library.', "firmasite-theme-enhancer" ),
			)
		);
	...
	public function render( $args, $instance ) {
		...
		if ( $inside ) {
			...
			$template_loader = new FirmaSite_Simple_Image_Widget_Template_Loader();
	...
	public function form( $instance ) {
		...
		<div class="simple-image-widget-form">
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', "firmasite-theme-enhancer" ); ?></label>
			...
			<?php if ( ! is_simple_image_widget_legacy() ) : ?>
				<p class="simple-image-widget-control<?php echo ( $image_id ) ? ' has-image' : ''; ?>"
					...
					<a href="#" class="<?php echo esc_attr( join( ' ', $button_class ) ); ?>"><?php _e( 'Choose an Image', "firmasite-theme-enhancer" ); ?></a>
				</p>
			<?php endif; ?>
					...
					switch ( $field ) {
						case 'image_size' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'image_size' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php _e( 'Size:', "firmasite-theme-enhancer" ); ?></label>
						...
						case 'link' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'new_window' ) ); ?>" style="margin-top: -0.75em; padding-left: 2px">
								<label for="<?php echo esc_attr( $this->get_field_id( 'new_window' ) ); ?>">
									...
									<?php _e( 'Open in new window?', "firmasite-theme-enhancer" ); ?>
								</label>
							</p>
							<?php
							break;
						...
						case 'link_classes' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_classes' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_classes' ) ); ?>"><?php _e( 'Link Classes:', "firmasite-theme-enhancer" ); ?></label>
						...
						case 'link_text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_text' ) ); ?>"><?php _e( 'Link Text:', "firmasite-theme-enhancer" ); ?></label>
						...
						case 'text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php _e( 'Text:', "firmasite-theme-enhancer" ); ?></label>
	...
	public function get_image_sizes( $image_id ) {
		$sizes = array( 'full' => __( 'Full Size', "firmasite-theme-enhancer" ) );

		$imagedata = wp_get_attachment_metadata( $image_id );
		if ( isset( $imagedata['sizes'] ) ) {
			$size_names = FirmaSite_Simple_Image_Widget_Plugin::get_image_size_names();
	...
	protected function siw_field_class( $id ) {
		...
		$hidden_fields = FirmaSite_Simple_Image_Widget_Plugin::get_hidden_fields();
