...
class FirmaSite_Widget_Conditions {
	...
	public static function widget_conditions_options_echo( $major = '', $minor = '' ) {
		switch ( $major ) {
			case 'category':
				?>
				<option value=""><?php _e( 'All category pages', "firmasite-theme-enhancer" ); ?></option>
				<?php
			...
			case 'loggedin':
				?>
				<option value="loggedin" <?php selected( 'loggedin', $minor ); ?>><?php _e( 'Logged In', "firmasite-theme-enhancer" ); ?></option>
				<option value="loggedout" <?php selected( 'loggedout', $minor ); ?>><?php _e( 'Logged Out', "firmasite-theme-enhancer" ); ?></option>
				<?php
			break;
			case 'author':
				?>
				<option value=""><?php _e( 'All author pages', "firmasite-theme-enhancer" ); ?></option>
				<?php
			...
			case 'tag':
				?>
				<option value=""><?php _e( 'All tag pages', "firmasite-theme-enhancer" ); ?></option>
				<?php
			...
			case 'date':
				?>
				<option value="" <?php selected( '', $minor ); ?>><?php _e( 'All date archives', "firmasite-theme-enhancer" ); ?></option>
				<option value="day"<?php selected( 'day', $minor ); ?>><?php _e( 'Daily archives', "firmasite-theme-enhancer" ); ?></option>
				<option value="month"<?php selected( 'month', $minor ); ?>><?php _e( 'Monthly archives', "firmasite-theme-enhancer" ); ?></option>
				<option value="year"<?php selected( 'year', $minor ); ?>><?php _e( 'Yearly archives', "firmasite-theme-enhancer" ); ?></option>
				<?php
			break;
			case 'page':
				...
				<optgroup label="<?php esc_attr_e( 'Static page:', "firmasite-theme-enhancer" ); ?>">
					<?php
	...
	public static function widget_conditions_admin( $widget, $return, $instance ) {
		...
		<div class="widget-conditional <?php if ( empty( $_POST['widget-conditions-visible'] ) || $_POST['widget-conditions-visible'] == '0' ) { ?>widget-conditional-hide<?php } ?>">
			...
			<?php if ( ! isset( $_POST['widget-conditions-visible'] ) ) { ?><a href="#" class="button display-options"><?php _e( 'Visibility', "firmasite-theme-enhancer" ); ?></a><?php } ?>
			<div class="widget-conditional-inner">
				<div class="condition-top">
					<?php printf( _x( '%s if:', 'placeholder: dropdown menu to select widget visibility; hide if or show if', "firmasite-theme-enhancer" ), '<select name="conditions[action]"><option value="show" ' . selected( $conditions['action'], 'show', false ) . '>' . esc_html_x( 'Show', 'Used in the "%s if:" translation for the widget visibility dropdown', "firmasite-theme-enhancer" ) . '</option><option value="hide" ' . selected( $conditions['action'], 'hide', false ) . '>' . esc_html_x( 'Hide', 'Used in the "%s if:" translation for the widget visibility dropdown', "firmasite-theme-enhancer" ) . '</option></select>' ); ?>
				</div><!-- .condition-top -->
						...
						<div class="condition">
							<div class="selection alignleft">
								<select class="conditions-rule-major" name="conditions[rules_major][]">
									...
									<option value="author" <?php selected( "author", $rule['major'] ); ?>><?php echo esc_html_x( 'Author', 'Noun, as in: "The author of this post is..."', "firmasite-theme-enhancer" ); ?></option>
									<?php
									// this doesn't work on .com because of caching
									if( ! ( defined( 'IS_WPCOM' ) && IS_WPCOM ) ) {
									?>
									<option value="loggedin" <?php selected( "loggedin", $rule['major'] ); ?>><?php echo esc_html_x( 'User', 'Noun', "firmasite-theme-enhancer" ); ?></option>
									<option value="role" <?php selected( "role", $rule['major'] ); ?>><?php echo esc_html_x( 'Role', 'Noun, as in: "The user role of that can access this widget is..."', "firmasite-theme-enhancer" ); ?></option>
									<?php } ?>
