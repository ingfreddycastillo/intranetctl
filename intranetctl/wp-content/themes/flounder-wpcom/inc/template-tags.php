...
function flounder_comment_link( $before = '', $after = '', $echo = true ) {
	...
	if ( ! empty( $before ) )
		echo $before;
	comments_popup_link( '<i class="icon dashicons dashicons-admin-comments"></i>'.__( '0 Comments', 'flounder' ), '<i class="icon dashicons dashicons-admin-comments"></i>'.__( '1 Comment', 'flounder' ), '<i class="icon dashicons dashicons-admin-comments"></i>'.__( '% Comments', 'flounder' ), 'read alignleft', '' );
	printf( '<a href="%s" class="add alignright"><i class="icon dashicons dashicons-plus-big"></i>%s</a>', get_permalink() . '#respond', __( 'Leave a comment', 'flounder' ) );
	if ( ! empty( $after ) )
		echo $after;
	if ( $echo ) {
		ob_end_flush();
